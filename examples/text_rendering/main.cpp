...
int main() {
  ...
  Shader ourShader("../examples/text_rendering/shader/vertex.glsl", "../examples/text_rendering/shader/fragment.glsl");
    ...
    glm::mat4 projection = glm::ortho(0.0f, static_cast<GLfloat>(gl_app->mWidth), 0.0f, static_cast<GLfloat>(gl_app->mHeight));
    ourShader.begin();
    glUniformMatrix4fv(glGetUniformLocation(ourShader, "projection"), 1, GL_FALSE, glm::value_ptr(projection));
    ...
    // Load font as face
    FT_Face face;
    if (FT_New_Face(ft, "../assets/simkai.ttf", 0, &face))
